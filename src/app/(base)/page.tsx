...
const AdvantagesCards = dynamic(
  () => import("@/components/sections/advantagesCards/advantagesCards"),
);
...
export default async function Home() {
  const [cases, services, stages, teamPhoto, seo] = await Promise.all([
    api.getCases(1, null),
    api.getServices(),
    api.getInteractionStages(),
    api.getTeamPhoto(),
    getCachedSeo(),
  ]);
  ...
  const content = {
    ...
    wordSwipeProps: { words: ["web", "seo", "ads"] },
  };
  ...
  return (
    <>
        <div className={"mainContainer"}>
          ...
          <Greeting
            greetingTextProps={{ textContent: content, firstLine: "div" }}
            className={"mainPage"}
          />
          <h1 className={s.hiddenHeader}>Создаем сайты для бизнеса</h1>
          <GreetingDescription className={s.greetingDescription} />
        </div>
        {teamPhoto && <AboutCard className={s.about} teamPhoto={teamPhoto} />}
        <AdvantagesCards />
        <div className={"mainContainer"}>
          <Cases cases={casesForSection || []} />
        </div>
        <ServicesLinksList
          linksData={formattedServices || []}
