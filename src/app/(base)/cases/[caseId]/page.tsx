...
export default async function Case({
  ...
  const { caseId } = await params;
  const [casePage, seo] = await Promise.all([
    api.getCase(caseId),
    getCachedSeo(caseId),
  ]);
    ...
    <>
      <div className={s.casePage}>
        ...
        <div className={s.mainInfoContainer}>
          <Picture
            src={casePage.images.main}
            alt=""
            fill
            containerProps={{ className: s.mainImgContainer }}
            priority
            sizes={"100vw"}
          />
          <div className={s.descriptionContainer}>
            <div
              ...
              dangerouslySetInnerHTML={{ __html: casePage.description }}
            ></div>
          </div>
        </div>
        <div className={clsx("mainContainer", s.taskBlock)}>
          <div className={s.taskHeader}>
            ...
            <div
              ...
              dangerouslySetInnerHTML={{ __html: casePage.task }}
            ></div>
          </div>
          <div className={s.images}>
            {smallImg && (
              <Picture
                ...
                fill
                sizes={
                  "(max-width: 767px) 228px,(max-width: 1425px) 547px,(max-width: 1905px) 392px,536px"
                }
            ...
            {mediumImg && (
              <Picture
                ...
                fill
                sizes={
                  "(max-width: 767px) 288px,(max-width: 1425px) 691px,(max-width: 1905px) 808px, 1104px"
                }
        ...
        {bigImg && (
          <Picture
            ...
            fill
            sizes={"100vw"}
            containerProps={{ className: s.bigImg }}
          />
        )}
        <div className={clsx("mainContainer", s.resultBlock)}>
          ...
          <div className={s.resultInfo}>
            ...
            <div>
              ...
              <div
                className={s.stackTech}
                dangerouslySetInnerHTML={{ __html: casePage["tech_stack"] }}
              ></div>
            </div>
            {caseLink}
          </div>
          <div
            ...
            dangerouslySetInnerHTML={{ __html: casePage["project_text"] }}
          ></div>
        </div>
      </div>
      {schema && (
