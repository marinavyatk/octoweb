import brushAnimation from "@/lotties/brush.json";
import dialogAnimation from "@/lotties/dialog.json";
import rocketAnimation from "@/lotties/rocket.json";
import eyeAnimation from "@/lotties/eye.json";

export const cards = [
  {
    icon: brushAnimation,
    header: "Просто о сложном",
    paragraph:
      'Переводим технические моменты на "человеческий язык", чтобы вы могли понимать окончательный результат.',
  },
  {
    icon: dialogAnimation,
    header: "Ясность коммуникаций",
    paragraph:
      "Вы всегда можете уточнить, как идет работа. В коммуникации участвуют основные сотрудники нашей студии.",
  },
  {
    icon: rocketAnimation,
    header: "Полная эффективность",
    paragraph:
      "Быстро включаемся в решение задачи и также оперативно справляемся с ней без бюрократических задержек.",
  },
  {
    icon: eyeAnimation,
    header: "Прозрачные цены",
    paragraph:
      "Ясное представление о составе расценок, цена фиксируется в договоре, без скрытых или неочевидных платежей",
  },
];
