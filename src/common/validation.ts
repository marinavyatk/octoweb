...
//common
export const requiredString = z.string().min(1, {message: 'Это обязательное поле'}).max(500, 'Максимум 500 символов');
export const optionalString = z.string().max(500, 'Максимум 500 символов');
export const checkboxGroupRequired = z.string().array().nonempty({message: 'Выберите хотя бы один вариант'})
